        ...
        <div class="card">
            <form action="{{ route('news.update', $data['id']) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="card-body">
                    ...
                    <input type="text" name="title" placeholder="title" value="{{ $data['title'] }}" id="title" class="form-control mb-3" required>

                    <label for="banner" class="form-label">banner</label>
                    <input type="file" name="banner" placeholder="banner" id="banner" class="form-control mb-3">

                    <label for="content" class="form-label">content</label>
                    <textarea name="content" id="content" class="form-control mb-3" required>{{ $data['content'] }}</textarea>
                </div>
